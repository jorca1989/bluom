...
export function SubscriptionProvider({ children }: { children: React.ReactNode }) {
  ...
  const refresh = async () => {
    ...
    // Final pro decision: admin OR RevenueCat OR Convex subscription flags
    const convexPro =
      convexUser?.subscriptionStatus === 'pro' || convexUser?.isPremium === true;

    const finalPro = isAdmin || rcPro || convexPro;
  ...
  useEffect(() => {
    ...
    refresh().catch(() => {
      // If RevenueCat is not configured yet, treat as free (unless admin/convex says pro)
      const convexPro =
        convexUser?.subscriptionStatus === 'pro' || convexUser?.isPremium === true;
      setStatus(isAdmin || convexPro ? 'pro' : 'free');
    });
