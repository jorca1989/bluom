...
export default function MeditationHub({ userId, onClose }: MeditationHubProps) {
  ...
  const startSessionMutation = useMutation(api.meditation.startSession);
  ...
  const displaySessions = useMemo(() => {
    ...
    if (!selectedCategory) return fallbackSessions;
    return fallbackSessions.filter((s) => s.category === selectedCategory);
  }, [sessions, selectedCategory, fallbackSessions]);

  const handleStartSession = async (session: any) => {
    try {
      const id = await startSessionMutation({
        userId,
      ...
      const msg = (e as any)?.message ? String((e as any).message) : 'Could not start session.';
      if (msg.toLowerCase().includes('limit')) {
        ...
        if (!isPro) {
          Alert.alert('Premium Required', 'Free users can do 3 meditations per day. Upgrade for unlimited access.', [
