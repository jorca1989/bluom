  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      >
        ...
        {!loading && recipes && recipes.length > 0 && (
          <View style={styles.recipesGrid}>
            {recipes.map((recipe) => {
              const locked = recipe.isPremium && !isPro;
              return (
                <TouchableOpacity
                  key={recipe._id}
                ...
                >
                  <View style={styles.recipeCardImageContainer}>
                    {recipe.imageUrl ? (
                      <Image source={{ uri: recipe.imageUrl }} style={styles.recipeCardImage} />
                    ) : (
        ...
        <Modal visible animationType="slide" onRequestClose={() => setSelectedRecipe(null)}>
          <SafeAreaView style={styles.detailContainer} edges={['top', 'bottom']}>
            ...
            >
              ...
              <View style={styles.card}>
                ...
                <TouchableOpacity
                  ...
                  onPress={async () => {
                    ...
                    if (selectedRecipe?.isPremium && !isPro) {
                      setShowUpgrade(true);
                      return;
                    }
              ...
              {!!selectedRecipe.ingredients?.length && !(selectedRecipe?.isPremium && !isPro) && (
                <View style={styles.card}>
              ...
              {!!selectedRecipe.instructions?.length && !(selectedRecipe?.isPremium && !isPro) && (
                <View style={styles.card}>
              ...
              {selectedRecipe?.isPremium && !isPro ? (
                <>
                  <View style={styles.lockedCard}>
                    ...
                    <View style={styles.lockedOverlay}>
                      ...
                      <Text style={styles.lockedText}>Upgrade to Pro to unlock the full recipe.</Text>
      ...
      <ProUpgradeModal
        ...
        message="This recipe is locked. Upgrade to Pro to unlock premium recipes, ingredients, and instructions."
...
const styles = StyleSheet.create({
  ...
  detailContainer: { flex: 1, backgroundColor: '#ebf2fe' },
  scrollView: { flex: 1 },
  header: {
  ...
  ingredientDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: '#2563eb' },
  ingredientText: { fontSize: 14, color: '#1e293b', flex: 1 },
  instructionItem: { flexDirection: 'row', marginBottom: 16, gap: 12 },
  instructionNumber: { width: 32, height: 32, borderRadius: 16, backgroundColor: '#2563eb', justifyContent: 'center', alignItems: 'center', flexShrink: 0 },
